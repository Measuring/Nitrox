...
using NitroxModel.Platforms.OS.Windows.Structs.Debug;

namespace NitroxModel.Platforms.OS.Windows
{
    /// <summary>
    ///     Lower-level wrapper for an OS process than normal <see cref="Process" /> to support memory access, debugging, all
    ///     loaded DLLs discovery and exported functions listing.
    ///     TODO: Make cross-platform by adding support for GDB (unix debugger) and LLDB (MacOS)
    /// </summary>
    public sealed class ProcessEx : IDisposable
    {
        ...
        /// </summary>
        public bool Is32Bit => is32Bit ??= !Native.IsWow64Process(Handle, out bool isWowProcess) ? !Environment.Is64BitOperatingSystem : isWowProcess;
        ...
        public IntPtr PebAddress
        {
            get
            {
                ...
                NtStatus queryStatus = Native.NtQueryInformationProcess(Handle, 0, ref pbi, Marshal.SizeOf(typeof(ProcessBasicInformation)), IntPtr.Zero);
        ...
        public string MainModuleFileName
        {
            get
            {
                ...
                return mainModuleFileName = Native.QueryFullProcessImageName(Handle);
            }
        }
        ...
        public static ProcessDebugger StartWithDebugger(string fileName, IEnumerable<(string, string)> environmentVariables = null, string workingDirectory = null, string commandLine = null, bool disposeProcess = true)
        {
            return ProcessDebugger.Create(() => StartInternal(fileName, true, environmentVariables, workingDirectory, commandLine), disposeProcess);
        }

        public static ProcessEx Start(string fileName, IEnumerable<(string, string)> environmentVariables = null, string workingDirectory = null, string commandLine = null)
        {
        ...
        {
            return Native.CreateRemoteThread(Handle, IntPtr.Zero, 0, start, parameter, 0, out _);
        }
        ...
        public bool Suspend()
        {
            ...
            Native.NtSuspendProcess(Handle);
            return true;
        }
        ...
        public ThreadContext64 GetThreadContext(int threadId)
        {
            return Native.GetThreadContext(OpenThread(threadId, ThreadAccess.GET_CONTEXT));
        }

        public bool SetThreadContext(int threadId, ThreadContext64 context)
        {
            return Native.SetThreadContext(OpenThread(threadId, ThreadAccess.SET_CONTEXT), context);
        }
        ...
        {
            Native.WriteProcessMemory(Handle, address, data, data.Length, out int written);
            if (flushInstructionCache)
            {
                Native.FlushInstructionCache(Handle, address, (uint)written);
            }
            return written;
        }
        ...
        private static ProcessEx StartInternal(string fileName, bool withDebugger, IEnumerable<(string, string)> environmentVariables = null, string workingDirectory = null, string commandLine = null)
        {
            ...
            try
            {
                created = Native.CreateProcess(fileName, // Use this if pointing to a file
        ...
        private SafeAccessTokenHandle OpenThread(int threadId, ThreadAccess requiredAccess)
        {
            if (!threadHandles.TryGetValue(threadId, out ThreadHandle thread) || !thread.Access.HasFlag(requiredAccess))
            {
                ...
                threadHandles[threadId] = thread = new ThreadHandle(threadId, Native.OpenThread(requiredAccess, false, (uint)threadId), requiredAccess);
            }
