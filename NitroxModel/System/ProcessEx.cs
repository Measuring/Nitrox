...
namespace NitroxModel.System
{
    ...
    public sealed class ProcessEx : IDisposable
    {
        ...
        private readonly Dictionary<int, ThreadHandle> threadHandles = new Dictionary<int, ThreadHandle>();
        ...
        public IntPtr this[IntPtr address] => new IntPtr(BitConverter.ToInt32(ReadMemory(address, is32Bit == true ? 4 : 8), 0));
        ...
        public static ProcessEx GetFirstProcess(string procName, Func<ProcessEx, bool> predicate, StringComparer comparer = null)
        {
            ...
            foreach (Process proc in Process.GetProcesses())
            {
                ...
                ProcessEx procEx = new ProcessEx(proc);
                if (!predicate(procEx))
                {
                    procEx.Dispose();
                    continue;
        ...
        {
            return Win32Native.GetThreadContext(OpenThread(threadId, ThreadAccess.GET_CONTEXT));
        }

        public bool SetThreadContext(int threadId, ThreadContext64 context)
        {
            return Win32Native.SetThreadContext(OpenThread(threadId, ThreadAccess.SET_CONTEXT), context);
        }
        ...
        private static ProcessEx StartInternal(string fileName, bool withDebugger, IEnumerable<(string, string)> environmentVariables = null, string workingDirectory = null, string commandLine = null)
        {
            ...
            StartupInfo startupInfo = new StartupInfo { cb = Marshal.SizeOf<StartupInfo>() };
        ...
        private Dictionary<string, Module> GetModules()
        {
            ...
            List<int> offsets = new List<int> { 0x20 };
        ...
        public sealed class Module
        {
            ...
            private ImageExportDirectory ExportDirectory
            {
                get
                {
                    ...
                    IntPtr exportTablePtr = new IntPtr(BaseAddress.ToInt64() + header.OptionalHeader.ExportTable.VirtualAddress);
            ...
            {
                Dictionary<string, ExportedFunction> result = new Dictionary<string, ExportedFunction>();
                ...
                for (int i = 0; i < ExportDirectory.NumberOfNames; i++)
                {
                    ...
                    IntPtr functionAddress = new IntPtr(process.ReadStruct<uint>(functionAddressesPtr, ordinal * 4));
        ...
        private sealed class ThreadHandle : IDisposable
        {
            ...
            public SafeAccessTokenHandle Handle { get; private set; }
            public ThreadAccess Access { get; private set; }
